...
nzv = nearZeroVar(data.sub, saveMetrics= TRUE)
nzv[nzv$nzv,]
nzv <- nearZeroVar(data.sub)
filteredDescr <- data.sub[, -nzv]
dim(filteredDescr)
...
alglist = c('glmboost','treebag')
set.seed(1)
model = caretList(price~.,data=data.sub,trControl=control,methodList = alglist,verbose=F)
...
set.seed(1)
sample = sample(1:nrow(data.sub),3000)
try = data.sub[sample,]
library(kernlab)
x = try[,-1]
x = scale(x,center=TRUE,scale=TRUE)
KPCA = kpca(x, kernel="rbfdot", kpar=list(sigma=10))
eig(KPCA)
plot(pcv(KPCA)[,c(100,1000)])
x = predict(KPCA,x)
x = x[,1:30]
y = try[.-1]
...
train = cbind(x,price=try$price)
train = as.data.frame(train)
library(gbm)
boost=gbm(price~.,data=train,distribution="gaussian",interaction.depth = 10,
          n.minobsinnode=10,keep.data = TRUE,n.trees = 5000,shrinkage = 0.005,n.cores=8)
train.pred = predict(boost,train,n.trees = 5000)
sqrt(mean((train.pred-train$price)^2))

y = data.sub[-sample,]
test = predict(KPCA,y[,-1])
test = test[,1:30]
test = cbind(test,price=y[,1])
test = as.data.frame(test)
test.pred = predict(boost,test,n.trees=5000)
sqrt(mean((train.pred-train$price)^2))

library(e1071)
SVM = svm(price~.,train, gamma=0.005,cost=100)
train.pred = predict(SVM,train)
sqrt(mean((train.pred-train$price)^2))
test.pred = predict(SVM,test)
sqrt(mean((test.pred-test$price)^2))
...
boost=gbm(price~.,data=train.sub,distribution="gaussian",interaction.depth = 10,
          n.minobsinnode=10,keep.data = TRUE,n.trees = 2000,shrinkage = 0.01,n.cores=8)
test.pred = predict(boost,test.sub,n.trees = 2000)
test.error = sqrt(mean((test.pred-test.sub$price)^2)); test.error
train.pred = predict(boost,train.sub,n.trees = 2000)
