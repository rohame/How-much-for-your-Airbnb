data = read.csv("C:/Users/luoyu/Desktop/Methods and Framework/7_kaggle/competition/analysisData.csv",stringsAsFactors = F)
names(data)
...
library(foreach)
library(glmnet)
x=model.matrix(price~.-1,data=train.sub)
y=train.sub$price
cv.lasso=cv.glmnet(x,y,alpha=1)
coef(cv.lasso)
train.sub1=data.sub[,c('price','cleaning_fee','neighbourhood_Bronx','neighbourhood_Brooklyn','neighbourhood_Manhattan','neighbourhood_Staten','longitude','room_apt','room_shared','accommodates','bathrooms','bedrooms','guests_included','minimum_nights','property_type','reviews_per_month','review_scores_location','review_scores_value','availability_30','availability_90','availability_365','air','shampoo','tv','gym','washer','dryer')]

#model
library(gbm)
boost=gbm(price~.,data=train.sub1,distribution="gaussian",interaction.depth = 10,
...
#scoring data
scoringData=read.csv("C:/Users/luoyu/Desktop/Methods and Framework/7_kaggle/competition/scoringData.csv",stringsAsFactors = F)
...
scoringData.sub = scoringData[,c('cleaning_fee','neighbourhood_Bronx','neighbourhood_Brooklyn','neighbourhood_Manhattan','neighbourhood_Staten','longitude','room_apt','room_shared','accommodates','bathrooms','bedrooms','guests_included','minimum_nights','property_type','reviews_per_month','review_scores_location','review_scores_value','availability_30','availability_90','availability_365','air','shampoo','tv','gym','washer','dryer')]
write.csv(scoringData.sub, 'cleaned_score_data.csv',row.names = F)

# construct submision from predictions
test.pred = predict(boost,newdata=scoringData.sub,n.trees = 5000)
